...
>
> 以下の時間値は2026-02-02時点の記録で、現行環境での再計測は確認できていません。現行コードでも`page.tsx`がスレッド本体をサーバー側で取得し、Client Componentの`ThreadPostsStream`が投稿一覧をSWRで取得する分割は確認できます。
...
### 理由
* 当時の処理は、SSR側で全てデータを取得し、レンダリングしていた。
    * （当時は `getThreadWithPosts()` を fetch して `.map()` でレンダリングしていたため、処理時間が大きいと予測した。）
