...
interface MorphSurfaceProps {
	...
	surfacePositionClassName?: string;
	overlayExpand?: boolean;
	showContentAnchorDot?: boolean;
	wrapContentWithForm?: boolean;

	// Render props
	renderDock?: (props: DockProps) => React.ReactNode;
...
export function MorphSurface({
	...
	surfaceClassName,
	dockClassName,
	surfacePositionClassName,
	overlayExpand = false,
	showContentAnchorDot = true,
	wrapContentWithForm = true,
	renderDock,
	renderTrigger,
	renderContent,
	renderIndicator,
	...
	const resolvedSurfacePositionClassName =
		surfacePositionClassName ?? (overlayExpand ? "bottom-0" : "bottom-8");
	const rootWidth =
		overlayExpand && collapsedWidth !== "auto"
			? collapsedWidth
			: hookExpandedWidth;
	const rootHeight = overlayExpand ? hookCollapsedHeight : hookExpandedHeight;
		...
		>
			<motion.div
				...
				className={cn(
					...
					overlayExpand ? "absolute left-1/2 -translate-x-1/2" : "relative",
					resolvedSurfacePositionClassName,
...
function MorphSurfaceDock() {
	...
	if (renderDock) {
		const customDock = renderDock({
			success,
			isOpen: showFeedback,
			onToggle: () => openFeedback(),
		...
		return (
			<footer
				className={cn(
					"flex items-center justify-center select-none whitespace-nowrap mt-auto h-[44px]",
					dockClassName,
				)}
			>
				{customDock}
...
>(({ onSuccess }, ref) => {
	...
	const body = (
		<>
			...
			{showFeedback && showContentAnchorDot && (
				<motion.div
					layoutId={`morph-surface-dot-${triggerLabel}`}
